...
struct Cell {
    ...
    void Save(wxDataOutputStream &dos, Cell *ocs, std::set<Cell *> &bmc) const {
        ...
        if (bmc.contains(const_cast<Cell *>(this))) {
            cellflags |= TS_BOOKMARK_MASK;
        }
        if (HasTextState()) {
    ...
    Cell *LoadGrid(wxDataInputStream &dis, int &numcells, int &textbytes, Cell *&ics, std::set<Cell *> &bmc) {
        int xs = dis.Read32();

                ...
                if (image.trefc) {
                    if(sys->jpegdefaultimageformat) {
                        ...
                        if (image.jpeg_data.empty()) {
                            ...
                            off_t beforejpeg = mos.TellO();
                            im.SaveFile(mos, wxBITMAP_TYPE_JPEG);
                            off_t afterjpeg = mos.TellO();
                            auto sz = afterjpeg - beforejpeg;
                            image.jpeg_data.resize(sz);
                            mos.SeekO(beforejpeg);
                            mos.CopyTo(image.jpeg_data.data(), image.jpeg_data.size());
                            mos.SeekO(afterjpeg);
                            mos.Close();
                        } else {
                        ...
                        if (image.png_data.empty()) {
                            ...
                            wxMemoryOutputStream mos;
                            off_t beforepng = mos.TellO();
                            im.SaveFile(mos, wxBITMAP_TYPE_PNG);
                            off_t afterpng = mos.TellO();
                            auto sz = afterpng - beforepng;
                            image.png_data.resize(sz);
                            mos.SeekO(beforepng);
                            mos.CopyTo(image.png_data.data(), image.png_data.size());
                            mos.SeekO(afterpng);
                            mos.Close();
                        } else {
    ...
    const wxChar* CopyImageToClipboard(Cell *&cell) {
        if (wxTheClipboard->Open()) {
            ...
            if (!cell->text.image->png_data.empty()) {
                wxCustomDataObject *pngimage = new wxCustomDataObject("image/png");
                ...
                wxCustomDataObject *jpegimage = new wxCustomDataObject("image/jpeg");

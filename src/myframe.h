    ...
    MyFrame(wxString exename, MyApp *_app)
        ...
        if (showtbar || mergetbar) {
            ...
            tb->AddControl(filter = 
                new wxSearchCtrl(tb, A_SEARCH, "", wxDefaultPosition, FromDIP(wxSize(120, 22)), wxTE_PROCESS_ENTER));
            filter->ShowSearchButton(true);
            filter->ShowCancelButton(true);
            // last binding will be called first
            filter->Bind(wxEVT_SEARCH, &MyFrame::OnSearchEnter, this);
            filter->Bind(wxEVT_SEARCH_CANCEL, &MyFrame::OnSearch, this);
            
            filter->Bind(wxEVT_TEXT_ENTER, &MyFrame::OnSearchEnter, this);
            filter->Bind(wxEVT_TEXT, &MyFrame::OnSearch, this);
